...
async function loadStores() {
  try {
    ...
    const REGION = "강릉시";  ////////////////////////////////////////////////// 지역별 추가할 때 수정할 코드

    allStores = (Array.isArray(data) ? data : [])
      .filter(store => store && typeof store === "object")
      .filter(store => (store.address || "").includes(REGION))
      .map(store => ({
        ...store,
      }));
...
async function addStore(status = 'active') {
  ...
  const REGION = "강릉시";  /////////////////////////////////////////////////////// 지역별 추가할 때 수정

// 다른 지역 매장 등록 차단
    if (!address.includes(REGION)) {
      alert(`현재 ${REGION} 관리자 페이지에서는 ${REGION} 매장만 등록할 수 있습니다.`);
      return;
    }
...
window.addEventListener("load", async () => {
  ...
  await loadStores();       // 홍천군 매장 먼저 로드
  loadAdminCoupons();       // 그 다음 홍천군 쿠폰 로드
